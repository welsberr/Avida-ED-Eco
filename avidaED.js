...
>>>>>>> decb6d0fe5591c9b599031e8837a04efd5a2a5f5
  ...
  if (typeof Worker !== "undefined") {
    ...
    userMsgLabel.textContent =
      "Sorry, your browser does not support Web workers and Avida won't run";
  }
  ...
  $(document.getElementById("organCanvas")).on("mousedown", function (evt) {
    av.post.addUser(
      "mousedown: organCanvas(" + evt.offsetX + ", " + evt.offsetY + ")"
    );
  ...
  $(av.dom.gridCanvas).on("mousedown", function (evt) {
    av.post.addUser(
      "mousedown: gridCanvas(" + evt.offsetX + ", " + evt.offsetY + ")"
    );
  ...
  $(document).on("mouseup touchend", function (evt) {
    ...
    if (av.debug.mouse)
      console.log(
        "AvidaED.js: mouse.UpGridPosX, y",
        av.mouse.UpGridPos[0],
        av.mouse.UpGridPos[1]
      );
    ...
    if ("parent" == av.mouse.Picked) {
      ...
      if ("gridCanvas" == evt.target.id || "trashCanImage" == evt.target.id) {
        av.grd.drawGridSetupFn(
          "on mouseup where evt.target.id=gridCanvas or trashCanImage"
        );
...
*/
  ...
  window.onerror = function (message, file, line, col, error) {
    ...
    av.post.postLogPara =
      "Please send the info below to Help us make Avida-ED better by clicking on the [Send] button";
    av.debug.sendLogPara1 =
      "The error is at the beginning and end of the session log in the text below.";
    av.debug.postEmailLabel =
      "Please include your e-mail if you would like feed back or are willing to further assist in debug";
    av.debug.postNoteLabel =
      "Please include any additional comments in the field below.";
    av.debug.postEmailLabel =
      "Please include your e-mail for feedback or so we can discuss the problem further";
    av.debug.error =
      "Error: " + message + " from " + file + ":" + line + ":" + col;
    av.debug.sendLogScrollBox =
      av.debug.error +
      "\n\n" +
      av.fio.mailAddress +
      "\n\n" +
  ...
  av.post.sendWindow = function (from) {
    console.log(
      from,
      "called av.post.sendWindow; used for database; not email"
    );
    ...
    av.dom.postUserInfoLabel.textContent =
  ...
  av.ui.sendLogEmailFn = function () {
    ...
    var link =
      ...
      "?subject=" +
      escape("Avida-ED session log") +
      "&body=" +
      escape(av.debug.sendLogScrollBox);
  ...
  */
  ...
  av.debug.finalizeDtail = function () {
    ...
    av.debug.dTail =
      "" +
      "\nmapHolder.client wd Ht = " +
      av.dom.mapHolder.clientWidth +
      "  " +
      av.dom.mapHolder.clientHeight +
      "\nmapHolder.scroll wd Ht = " +
      av.dom.mapHolder.scrollWidth +
      "  " +
      av.dom.mapHolder.scrollHeight +
      "\n  gridHolder.client wd Ht = " +
      av.dom.gridHolder.clientWidth +
      "  " +
      av.dom.gridHolder.clientHeight +
      "\n  gridHolder.scroll wd Ht = " +
      av.dom.gridHolder.scrollWidth +
      "  " +
      av.dom.gridHolder.scrollHeight +
      "\n  gridCanvas.client wd Ht = " +
      av.dom.gridCanvas.clientWidth +
      "  " +
      av.dom.gridCanvas.clientHeight +
      "\n  gridCanvas.scroll wd Ht = " +
      av.dom.gridCanvas.scrollWidth +
      "  " +
      av.dom.gridCanvas.scrollHeight +
      "\n" +
      av.debug.dTail;
  };
...
>>>>>>> decb6d0fe5591c9b599031e8837a04efd5a2a5f5
  ...
  document.getElementById("mnFlSaveAs").onclick = function () {
    if (!av.brs.isSafari) {
      ...
      av.fio.userFname = prompt(
        "Choose a new name for your Workspace now",
        suggest
      );
...
>>>>>>> decb6d0fe5591c9b599031e8837a04efd5a2a5f5
  ...
  av.ui.aboutAvidaED = function (from) {
    ...
    var num =
      $("#aboutAvidaED_content").height() -
      ($("#aboutAvidaED_grid_container").height() + 80);
    console.log("ht = ", num);
  ...
  av.ui.language = function (domObj) {
    ...
    console.log(
      "not yet implemented: the language requested is:",
      domObj.value
    );
  };
  ...
  av.ui.feedback = function () {
    ...
    av.debug.postStatus = "";
    av.post.postLogPara =
      "Please send your comment, suggestion or problem to " +
      av.fio.mailAddress;
    av.debug.postNoteLabel =
      "Please describe the problem or suggestion in the comment field below. ";
    av.debug.postEmailLabel =
      "Please include your e-mail so we can discuss your problem or suggeston further.";
    av.dom.postError.style.color = "grey";

    // sendLogParagraph lines
    av.debug.sendLogPara1 =
      "Please send your comment, suggestion or problem to Avida-ED-development@googlegroups.com";

    av.debug.sendLogPara2 =
      "If there was an error or odd behavior in this session, include the session log, which is in the text box below. " +
  ...
  av.dom.newDishDiscard.onclick = function () {
    ...
    if (!av.dnd.userDraggedNewConfig) {
      ...
      av.dnd.loadConfigByNameFn(
        av.fzr.actConfig.name,
        "av.dom.newDishDiscard.onclick"
      ); // reload the current active config
    }
    av.dnd.userDraggedNewConfig = false;
  ...
  av.dom.newDishSaveWorld.onclick = function () {
    ...
    if (!av.dnd.userDraggedNewConfig) {
      ...
      av.dnd.loadConfigByNameFn(
        av.fzr.actConfig.name,
        "av.dom.newDishSaveWorld.onclick"
      ); // reload the current active config
    }
    av.dnd.userDraggedNewConfig = false;
  ...
  av.dom.newDishSaveConfig.onclick = function (domObj) {
    ...
    if (!av.dnd.userDraggedNewConfig) {
      ...
      av.dnd.loadConfigByNameFn(
        av.fzr.actConfig.name,
        "av.dom.newDishSaveConfig.onclick"
      ); // reload the current active config
    }
    av.dnd.userDraggedNewConfig = false;
  ...
  av.ui.newButtonBothFn = function () {
    ...
    if ("prepping" == av.grd.runState) {
      ...
      // check to see about saving current population
      av.ptd.makePauseState();
      console.log(
        "av.ui.newButtonBothFn: av.dom.newDishModalID=",
        av.dom.newDishModalID
      );
  ...
  av.dom.freezeButton.onclick = function () {
    console.log(
      "in av.dom.freezeButton.onclick: av.grd.runState=",
      av.grd.runState,
      "; av.msg.ByCellIDgenome=",
      av.msg.ByCellIDgenome,
      "; length=",
      av.msg.ByCellIDgenome.length
    );
    ...
    if ("prepping" == av.grd.runState)
      ...
      if (5 > av.msg.ByCellIDgenome.length) {
        document.getElementById("fzDialogModFzOrganismSpan").style.display =
          "none";
      } else
        document.getElementById("fzDialogModFzOrganismSpan").style.display =
          "inline";
...
*/
  ...
  av.ui.mainBoxSwap = function (showBlock) {
    ...
    if ("organismBlock" == av.ui.page) {
      ...
      console.log(
        "allAvidaContainer.class=",
        document.getElementById("allAvidaContainer").className
      );
    ...
    console.log(
      "In: av.ui.mainBoxSwap; av.pch.needInit=",
      av.pch.needInit,
      '; $(av.dom.popStatsBlock).is(":visible")=',
      $(av.dom.popStatsBlock).is(":visible")
    );
  ...
  av.dom.organismButton.onclick = function () {
    ...
    console.log(
      "orgInfoHolder.scrollWidth, client, offset =",
      av.dom.orgInfoHolder.scrollWidth,
      av.dom.orgInfoHolder.clientWidth,
      av.dom.orgInfoHolder.offsetWidth,
      "; $width, $innerWidth, $outerWidth, css(width)=",
      $("#orgInfoHolder").width(),
      $("#orgInfoHolder").innerWidth(),
      $("#orgInfoHolder").outerWidth(),
      $("#orgInfoHolder").css("width")
    );
    if (av.dom.orgInfoHolder.clientWidth < av.ui.orgInfoHolderMinWidth)
      av.ui.orgInfoHolderWidth = av.ui.orgInfoHolderMinWidth;
    av.ui.mainBoxSwap("organismBlock");
    resizeOrganismPage();
  ...
  av.ptd.rightInfoPanelToggleButton = function (domObj) {
    ...
    if ("populationBlock" == av.ui.page) {
      ...
      if ("SetupButton" == domObj.id) {
        document.getElementById("SetupButton").className =
          "toggleRitButton activeBtn";
        ...
        if ($(av.dom.popStatsBlock).is(":visible")) {
          if (av.dbg.flg.pch) {
            console.log("popChrt: need to call av.grd.popChartInit");
          }
  ...
  av.dom.runStopButton.onclick = function () {
    av.post.addUser(
      "Button: runStopButton = " + av.grd.updateNum,
      "=updateNum;  " +
        av.grd.msg.update +
        "=msg.update;  " +
        av.grd.popStatsMsg.update +
        "=popStatsMsg.update"
    );
    av.ptd.runStopFn();
  };
  ...
  av.ui.avidianOutlineOnclick = function (domObj) {
    av.post.addUser(
      "Button: avidianOutline; av.ui.showOutlineFlag=" + av.ui.showOutlineFlag
    );
    if (
      "Resource Mode: hide Avidian Outlines" == av.dom.avidianOutline.innerHTML
    ) {
  ...
  av.grd.drawGridSetupFn = function (from) {
    ...
    if (av.dbg.flg.dsz) {
      console.log(
        from,
        "called av.grd.drawGridSetupFn__________________________________________________"
      );
    }
    ...
    if ("populationBlock" === av.ui.page) {
      ...
      if ("none" == document.getElementById("colorMode").value.toLowerCase()) {
        if (av.grd.newlyNone) {
          ...
          av.grd.cntx.fillRect(
            1,
            1,
            av.dom.gridCanvas.width - 1,
            av.dom.gridCanvas.height - 1
          );
        }
      } else {
        ...
        if (undefined != av.grd.msg) {
          if (
            "prepping" != av.grd.runState &&
            undefined != av.grd.msg.fitness
          ) {
        ...
        av.dom.scaleCanvas.width =
          $("#sclCnvsHldr").width() + 0.5 * $("#sclCnvsHldr").width();

        //figure out scale or legend
        if ("Ancestor Organism" == document.getElementById("colorMode").value) {
          av.grd.drawLegend();
        } else {
          av.grd.setColorMapOnly(
            "draw gradient scale in av.grd.drawGridSetupFn"
          ); //to set color scales for resources
          av.grd.gradientScale("av.grd.drawGridSetupFn");
        }

        if (av.dbg.flg.dsz) {
          console.log("dsz: scaleCanvas ht =", av.dom.scaleCanvas.height);
        }
        ...
        if (av.dbg.flg.dsz) {
          console.log("dsz: ", $("#sclCnvsHldr").height(), "= sclCnvsHldr ht");
        }

        if (av.dbg.flg.dsz) {
          console.log(
            "--------------- gridHolder ht =",
            $("#gridHolder").height().toFixed(1)
          );
        }
        //if (av.dbg.flg.dsz) { console.log('dsz: before: benchPopBot.scroll.Height =', document.getElementById('benchPopBot').scrollHeight + 'px'); }
        document.getElementById("benchPopBot").style.height =
          document.getElementById("benchPopBot").scrollHeight + "px";
  ...
  av.pch.popChrtYaxisChangeFn = function (domobj) {
    ...
    console.log(
      "popChrtYaxisChangeFn: domobj.id=",
      domobj.id,
      "; substring=",
      side
    );
  ...
  av.grd.popChartInit = function (from) {
    ...
    if (av.dbg.flg.pch) {
      console.log(
        "popChrt",
        from,
        "called av.grd.popChartInit; av.pch.needInit=",
        av.pch.needInit,
        "; av.ui.page=",
        av.ui.page,
        '; $(av.dom.popStatsBlock).is(":visible")=',
        $(av.dom.popStatsBlock).is(":visible"),
        "; av.dom.popStatsBlock.style =",
        av.dom.popStatsBlock.style
      );
    }
    ...
    if (true) {
      ...
      if (undefined != av.dom.popChart.data[0]) {
        ...
        if (av.dbg.flg.pch) {
          console.log("PopPlot: av.dom.popChart.data=", av.dom.popChart.data);
        }
      }
    }
  ...
  av.grd.popChartFn = function (from) {
    ...
    if (av.dbg.flg.pch) {
      console.log(
        "popChrt:",
        from,
        "called popChartFn: $(statsBlock.display = ",
        $(av.dom.popStatsBlock).css("display"),
        "; av.ui.page=",
        av.ui.page
      );
    }
    ...
    if (av.pch.needInit) {
      if (av.dbg.flg.pch) {
        console.log(
          from + " called av.grd.popChartFn: av.pch.needInit= ",
          av.pch.needInit,
          "; av.ui.page =",
          av.ui.page
        );
      ...
      if ($(av.dom.popStatsBlock).is(":visible")) {
        ...
        if (av.dbg.flg.pch) {
          console.log("popChrt: av.grd.runState = ", av.grd.runState);
        }
      }
    }
    ...
    if (!$(av.dom.popStatsBlock).is(":visible")) {
      if (av.dbg.flg.pch) {
        console.log("popChrt: Not visible: so skip rest of function");
      }
      return;
    }
    ...
    if (av.pch.lftYaxisWas != av.pch.lftYaxisNow) {
      av.pch.lftYchange = true;
    } else {
      av.pch.lftYchange = false;
    }
    if (av.pch.ritYaxisWas != av.pch.ritYaxisNow) {
      av.pch.ritYchange = true;
    } else {
      av.pch.ritYchange = false;
    }
    ...
    if ("None" !== av.pch.lftYaxisNow) {
      ...
      if (true) {
        ...
        if (av.dbg.flg.pch) {
          console.log("PopChrt: av.pch.aveFit = ", av.pch.aveFit);
        }
        if (av.dbg.flg.pch) {
          console.log("PopChrt: av.pch.logFit = ", av.pch.logFit);
        }
        if (av.dbg.flg.pch) {
          console.log(
            "PopChrt: av.pch.maxFit =",
            av.pch.aveMaxFit,
            "; av.pch.logMaxFit=",
            av.pch.logMaxFit,
            "~~~~~~~~~~~~~~~"
        ...
        switch (av.pch.lftYaxisNow) {
          case "Average Fitness":
            ...
            av.pch.maxY =
              av.pch.aveMaxFit > av.pch.logMaxFit
                ? av.pch.aveMaxFit
                : av.pch.logMaxFit;
            if (av.dbg.flg.pch) {
              console.log(
          ...
          case "Average Offspring Cost":
            ...
            av.pch.maxY =
              av.pch.aveMaxCst > av.pch.logMaxCst
                ? av.pch.aveMaxCst
                : av.pch.logMaxCst;
            if (av.dbg.flg.pch) {
              console.log(
          ...
          case "Average Energy Acq. Rate":
            ...
            av.pch.maxY =
              av.pch.aveMaxEar > av.pch.logMaxEar
                ? av.pch.aveMaxEar
                : av.pch.logMaxEar;
            break;
          case "Number of Organisms":
            ...
            av.pch.maxY =
              av.pch.aveMaxNum > av.pch.logMaxNum
                ? av.pch.aveMaxNum
                : av.pch.logMaxNum;
            break;
          case "Number Viable":
            ...
            av.pch.logY = av.pch.logNum;
            av.pch.maxY =
              av.pch.aveMaxVia > av.pch.logMaxNum
                ? av.pch.aveMaxVia
                : av.pch.logMaxNum;
          default:
            av.pch.yValue = "none";
      ...
      if (av.dbg.flg.pch) {
        console.log("PopChrt: lft   xx=", av.pch.xx);
      }
      if (av.dbg.flg.pch) {
        console.log("PopChrt: lft popY=", av.pch.popY);
      }
      if (av.dbg.flg.pch) {
        console.log("PopChrt: lft logY=", av.pch.logY);
      }
      if (av.dbg.flg.pch) {
        console.log("PopChrt: lft dadY=", av.pch.dadY);
      }
      if (av.dbg.flg.pch) {
        console.log(
          "PopChrt: lft maxY=",
    ...
    if (av.dbg.flg.pch) {
      console.log(
        ...
        "; av.pch.popData=",
        av.pch.popData,
        "; av.pch.ritYaxisNow=" + av.pch.ritYaxisNow
      );
    }
    ...
    if (true) {
      ...
      if (av.debug.uil) {
        console.log(
          "ui: av.pch.layout.wd ht=",
          av.pch.layout.width,
          av.pch.layout.height
        );
      }
      ...
      if (null == av.dom.popChart.data || 0 == av.dom.popChart.data.length) {
        ...
        console.log(
          "?? data is null or length 0; av.dom.popChart.data is",
          av.dom.popChart.data
        );
        ...
        console.log(
          "after Plotly.plot; av.dom.popChart.data is",
          av.dom.popChart.data
        );
        ...
        if (av.brs.isChrome) {
          if (av.dbg.flg.pch) {
            console.log("PopChrt: true? av.brs.isChrome=", av.brs.isChrome);
          }
          for (var ii = 0; ii < numof; ii++) {
            av.debug.log +=
              '\n     --uiD: Plotly: Plotly.restyle("popChart", av.pch.traceList[' +
              ii +
              "], [" +
              ii +
              "])";
          ...
          av.utl.dTailWrite(
            "AvidaED.js",
            new Error().lineNumber,
            "av.dom.popChart, av.pch.update",
            [av.dom.popChart, av.pch.update]
          );
          Plotly.relayout(av.dom.popChart, av.pch.update);
        } else {
          if (av.dbg.flg.pch) {
            console.log("PopChrt: false? av.brs.isChrome=", av.brs.isChrome);
          }
          ...
          Plotly.relayout(av.dom.popChart, av.pch.update);
          console.log(
            "after relayout in update grid chart: av.dom.popChart=",
            av.dom.popChart,
            "; av.pch.update=",
            av.pch.update
          );
          ...
          if (av.dbg.flg.pch) {
            console.log(
              "popChrt: before Plotly.animate; av.pch.popData",
              av.pch.popData
            );
          }
          av.debug.log +=
          ...
          if (av.dbg.flg.pch) {
            console.log("popChrt: before Plotly.animate; popData", popData);
          }
          if (av.dbg.flg.pch) {
            console.log(
              "popChrt: before Plotly.animate; av.dom.popChart=",
              av.dom.popChart
            );
          }
          ...
          if (av.dbg.flg.pch) {
            console.log(
              "popChrt: after Plotly.animate; av.dom.popChart=",
              av.dom.popChart
            );
          }
      ...
      if (av.dbg.flg.pch) {
        console.log("popChrt: av.dom.popChart.data=", av.dom.popChart.data);
      }
      if (av.dbg.flg.pch) {
        console.log("popChrt: av.dom.popChart.layout=", av.dom.popChart.layout);
      }
    } //end of true
  };
  ...
  av.ptd.popSizeFn = function (from) {
    ...
    av.dom.sizeCells.innerHTML =
      "for a total of " + av.grd.setupCols * av.grd.setupRows + " cells";
    ...
    if (undefined != av.parents.handNdx) {
      ...
      for (var ii = 0; ii < lngth; ii++) {
        if (av.debug.popCon) {
          console.log(
            "old cr",
            av.parents.col[av.parents.handNdx[ii]],
            av.parents.row[av.parents.handNdx[ii]]
          );
        }
        av.parents.col[av.parents.handNdx[ii]] = Math.floor(
          (av.grd.setupCols * av.parents.col[av.parents.handNdx[ii]]) /
            av.grd.gridWasCols
        ); //was trunc
        av.parents.row[av.parents.handNdx[ii]] = Math.floor(
          (av.grd.setupRows * av.parents.row[av.parents.handNdx[ii]]) /
            av.grd.gridWasRows
        ); //was trunc
        av.parents.AvidaNdx[av.parents.handNdx[ii]] =
          av.parents.col[av.parents.handNdx[ii]] +
          av.grd.setupCols * av.parents.row[av.parents.handNdx[ii]];
        if (av.debug.popCon) {
          console.log(
            "New cr",
            av.parents.col[av.parents.handNdx[ii]],
  ...
  $(function slidePopMute() {
    ...
    var slides = $("#mutePopSlide").slider({
      ...
      slide: function (event, ui) {
        ...
        if (10 <= tmpVal) {
          tmpVal = tmpVal.toFixed(0);
        } //had been 12, but 10 is more consistent with 2 sig figs
        else if (1 <= tmpVal) {
          tmpVal = tmpVal.toFixed(1);
        } else if (0.3 <= tmpVal) {
    ...
    $("#mutePopInput").change(function () {
      ...
      if (muteNum >= 0 && muteNum <= 100) {
        ...
        av.post.addUser(
          "mutePopInput =" + av.dom.mutePopInput.value,
          "1add ? 949"
        );
      } else {
        ...
        if (muteNum <= 0) {
          av.dom.mutePopError.innerHTML +=
            "Mutation rate must be >= than zero percent. ";
          if (av.debug.popCon) {
            console.log("<0");
          }
        }
        if (muteNum >= 100) {
          av.dom.mutePopError.innerHTML +=
            "Mutation rate must be 100% or less. ";
          if (av.debug.popCon) {
            console.log(">0");
          }
        }
        if (isNaN(muteNum)) {
          av.dom.mutePopError.innerHTML +=
            "Mutation rate must be a valid number. ";
          if (av.debug.popCon) {
            console.log("==NaN");
          }
        }
...
>>>>>>> decb6d0fe5591c9b599031e8837a04efd5a2a5f5
  ...
  av.ptd.gridChange = function (domObj) {
    ...
    if (colNum > 0 && colNum <= 100 && rowNum > 0 && rowNum <= 100) {
      ...
      if (rowNum <= 0) {
        av.dom.sizeCells.innerHTML +=
          "Number rof rows must be greater than zero. ";
        // if (av.dbg.flg.popSetup ) { console.log('popSetup: <0'); }
      }
      if (rowNum >= 100) {
  ...
  av.ptd.gridChangTest = function (from) {
    ...
    if (colNum > 0 && colNum <= 100 && rowNum > 0 && rowNum <= 100) {
      ...
      if (colNum >= 100) {
        av.dom.sizeCellTest.innerHTML +=
          "Number of columns must be 100 or less. ";
        //console.log('>0');
      }
      if (isNaN(colNum)) {
        av.dom.sizeCellTest.innerHTML +=
      ...
      if (isNaN(rowNum)) {
        av.dom.sizeCellTest.innerHTML +=
          "Number of rows must be a valid number. ";
        //console.log('==NaN');
      }
    }
  };
...
>>>>>>> decb6d0fe5591c9b599031e8837a04efd5a2a5f5
  ...
  av.ui.adjustOrgInstructionTextAreaSize = function () {
    var height =
      ($("#orgInfoHolder").innerHeight() -
        $("#orgDetailID").innerHeight() -
        10) /
      2;
  ...
  $(function slideOrganism() {
    ...
    $("#orgMuteInput").change(function () {
      ...
      if (muteNum >= 0 && muteNum <= 100) {
        ...
        av.post.addUser(
          "orgMuteInput =" + document.getElementById("orgMuteInput").value,
          "1add ? 949"
        );
      } else {
        av.ptd.validMuteInuput = false;
  ...
  av.ind.updateOrgTrace = function (from) {
    ...
    if (
      !(
        undefined == av.traceObj ||
        {} == av.traceObj ||
        undefined == av.traceObj[av.ind.cycle]
      )
    ) {
  ...
  document.getElementById("orgEnd").onclick = function () {
    if (av.debug.ind) {
      console.log(
        'orgEnd: av.ind.cycleSlider.get("maximum)',
        av.ind.cycleSlider.get("maximum")
      );
    }
  ...
  if (av.debug.ind) {
    console.log(
      "bevore av.ind.cycleslider av.dom.cycleSlider =",
      av.dom.cycleSlider
    );
  }
  //----------------------------------------------------------------------------------------------------------------------
  ...
  av.anl.anaChartInit = function () {
    ...
    if (undefined !== av.dom.anlChrtSpace.data) {
      if (av.dbg.flg.divsize) {
        console.log("AnaPlot: before purge in init");
      }
      av.debug.log +=
      ...
      if (av.dbg.flg.divsize) {
        console.log("AnaPlot: after purge in init");
      }
    }
    //Comment out the next three lines later
    var anaData = av.anl.data;
    if (av.dbg.flg.divsize) {
      console.log("AnaPlot: anlChrtPlotly in av.anl.anaChartInit");
    }
    console.log(
      "AnaPlot: anlChrtPlotly in av.anl.anaChartInit before first Plotly call"
    );
    ...
    av.utl.dTailWrite(
      "avidaED.js",
      new Error().lineNumber,
      "av.dom.anlChrtSpace, anaData, av.anl.layout, av.anl.widg",
      [av.dom.anlChrtSpace, anaData, av.anl.layout, av.anl.widg]
    );
    Plotly.plot(av.dom.anlChrtSpace, anaData, av.anl.layout, av.anl.widg);
    if (av.dbg.flg.divsize) {
      console.log("AnaPlot: after plot in av.anl.anaChartInit");
    }
  ...
  av.anl.AnaChartFn = function (from) {
    ...
    if (!hasData) av.dom.anlChrtSpace.style.visibility = "hidden";
      ...
      if (
        "none" === document.getElementById("yLeftSelect").value &&
        "none" === document.getElementById("yRightSelect").value
      ) {
        if (av.dbg.flg.plt) {
          console.log("AnaPlot: both axis set to none");
        }
        ...
        if (av.dbg.flg.plt) {
          console.log("AnaPlot: in AnaChartFn");
        }
        ...
        av.anl.trace0.name =
          av.anl.abbreviate[document.getElementById("yLeftSelect").value] +
          "-" +
          document.getElementById("popDish0").textContent;
        av.anl.trace1.name =
          av.anl.abbreviate[document.getElementById("yRightSelect").value] +
          "-" +
          document.getElementById("popDish0").textContent;
        av.anl.trace2.name =
          av.anl.abbreviate[document.getElementById("yLeftSelect").value] +
          "-" +
          document.getElementById("popDish1").textContent;
        av.anl.trace3.name =
          av.anl.abbreviate[document.getElementById("yRightSelect").value] +
          "-" +
          document.getElementById("popDish1").textContent;
        av.anl.trace4.name =
          av.anl.abbreviate[document.getElementById("yLeftSelect").value] +
          "-" +
          document.getElementById("popDish2").textContent;
        av.anl.trace5.name =
          av.anl.abbreviate[document.getElementById("yRightSelect").value] +
          "-" +
          document.getElementById("popDish2").textContent;
        ...
        if (av.dbg.flg.plt) {
          console.log("AnaPlot: av.anl.xx", av.anl.xx);
        }
        if (av.dbg.flg.plt) {
        ...
        av.anl.layout.yaxis.title =
          document.getElementById("yLeftSelect").value;
        av.anl.layout.yaxis2.title =
          document.getElementById("yRightSelect").value;
        if (av.dbg.flg.plt) {
          console.log("AnaPlot: before purge in update");
        }
        av.debug.log +=
        ...
        Plotly.purge(av.dom.anlChrtSpace);
        if (av.dbg.flg.plt) {
          console.log("AnaPlot: after plot anlChrtSpace");
        }
        //Plotly.plot('anlChrtSpace', anaData, av.anl.layout, av.anl.widg);
        av.debug.log +=
  ...
  av.anl.loadSelectedData = function (worldNum, axisSide, side, from) {
    ...
    console.log(
      from,
      "called v.anl.loadSelectedData: worldNum=",
      worldNum,
      "; axis=",
      axisSide,
      "; side=",
  ...
  */
  ...
  if (false) {
    console.log(
      "testing mode; set to false before public release for Avida-ED 4.0.14 Beta Testing. "
    );
  ...
  av.pch.pop_statsBlock_ChrtHolder_noResrceGrid =
    av.pch.popStatsBlock_Ht - av.pch.popChrtHolder_Ht;
  ...
  var ro = new ResizeObserver((entries) => {
    ...
    for (let entry of entries) {
      ...
      if (av.dbg.flg.dsz) {
        console.log(
          entry.target.id,
          `size wd, ht: ${cr.width}px  ${cr.height}px`
        );
      }
      if (av.dbg.flg.dsz) {
      ...
      if (av.dbg.flg.dsz) {
        console.log(
          ...
          cr.width - cr.left,
          cr.height - cr.top,
          "might need to multiply left and top by two"
        );
      }
    }
  });
...
av.pch.divSize = function (from) {
  //av.debug.uil = true;
  if (av.debug.uil) {
    console.log("ui: PopPlotSize: ", from, "called av.pch.divSize");
  }
  if (av.debug.uil) {
    console.log(
      "ui: popChrtHolder css.wd ht border padding margin=",
      $("#popChrtHolder").css("width"),
      $("#popChrtHolder").css("height"),
      $("#popChrtHolder").css("border"),
      $("#popChrtHolder").css("padding"),
      $("#popChrtHolder").css("margin")
    );
  }
  if (av.debug.uil) {
    console.log(
      "ui: PopPlotSize: av.dom.popChrtHolder.ht offset, client ht=",
      av.dom.popChrtHolder.offsetHeight,
      av.dom.popChrtHolder.clientHeight,
      "; parseInt(padding)=",
      parseInt($("#popChrtHolder").css("padding"), 10)
    );
  }
  ...
  if (av.debug.uil) {
    console.log(
      "ui: PopPlotSize: av.pch.pixel.wd ht=",
      av.pch.pixel.wd,
      av.pch.pixel.ht
    );
  }
  if (av.debug.uil) {
    console.log(
      "ui: PopPlotSize: av.pch.layout.wd ht=",
      av.pch.layout.width,
      av.pch.layout.height
    );
  }
  ...
  if (av.debug.uil) {
    console.log(
      "ui: PopPlotSize: popChart css.wd, border, padding, margin=",
      $("#popChart").css("width"),
      $("#popChart").css("height"),
      $("#popChart").css("border"),
      $("#popChart").css("padding"),
      $("#popChart").css("margin")
    );
  }
  if (av.debug.uil) {
    console.log(
      "ui: PopPlotSize: av.dom.popChart.ht offset, client ht=",
      av.dom.popChart.offsetHeight,
      av.dom.popChart.clientHeight,
      "; parseInt(padding)=",
      parseInt($("#popChart").css("padding"), 10)
    );
  }
  if (av.debug.uil) {
    console.log(
